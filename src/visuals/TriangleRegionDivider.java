...
/**
 * This is the class that divides up the screen up into Triangles
 * 
 * @author Jeremy
 *
 */
	...
	protected void assignPoints(Polygon myShape, int i, int j, int count) {
		...
		if (count % 2 != 1)
			myShape.getPoints().addAll(
					new Double[] {
							(double) getXOffset() + (x - rowShift(j))
									* getCellWidth(),
							(double) getYOffset() + y * getCellHeight(),
							(double) getXOffset() + (x + 1 - rowShift(j))
									* getCellWidth(),
							(double) getYOffset() + y * getCellHeight(),
							(double) getXOffset() + (x + .5 - rowShift(j))
									* getCellWidth(),
			...
			myShape.getPoints()
					.addAll(new Double[] {
							...
							(double) getXOffset() + (x + .5 - rowShift(j))
									* getCellWidth(),
							(double) getYOffset() + (y + 1) * getCellHeight(),
							(double) getXOffset() + (x + 1.5 - rowShift(j))
									* getCellWidth(),
							(double) getYOffset() + (y + 1) * getCellHeight() });
	}
